...
- **Reframed the project as FDE Delivery OS.** The GitHub repository and project were renamed from `FDE-Metodologia-A.C.E.S` to `FDE-Delivery-OS`. The A.C.E.S. methodology remains the system's kernel (phases unchanged); the product name now reflects the larger ambition: an agentic delivery operating system, not just a documented methodology.
...
- Added the **execution agent team** (`.claude/agents/`): 8 agents — `fde-master` (orchestrator), `fde-qualifier`, `fde-assessor`, `fde-context-engineer`, `fde-architect`, `fde-guardrails`, `fde-qa`, `fde-scale-ops` — each mapped to a phase or discipline of the A.C.E.S. methodology.
...
- Initial repository structure with the A.C.E.S. methodology.
